...
function VectorStoreForm() {
  ...
  return (
    <form
      className="flex min-h-0 min-w-0 flex-1 flex-col gap-3.5 overflow-y-auto"
      onSubmit={(e) => {
        e.preventDefault();
        void store.save();
      }}
    >
      <section className="flex flex-none flex-col gap-3.25 rounded-card border border-main-700 bg-main-900 p-4">
        ...
        <Field label="Описание" htmlFor="vs-description" optionalHint error={vm.errors.description}>
          <TextArea
            id="vs-description"
      ...
      <section className="flex flex-none flex-col gap-3.25 rounded-card border border-main-700 bg-main-900 p-4">
        ...
        <p className="text-[11px] leading-relaxed text-main-500">
          LanceDB · FLAT. Провайдер, модель, размерность и метрика фиксируются при создании. Размер
          чанка и перекрытие можно менять до индексации.
        </p>
